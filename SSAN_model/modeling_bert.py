...
def mish(x):
    ...
    #mish激活函数公式：f(x)=x*tanh(ln(1+ex)),其中ln(1+ex）--softplus
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))   #得到初始注意力分数
        ...
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)   #加入注意偏差后的注意力分数
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
            attention_scores = attention_scores + attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores)  #add&norm 归一化注意力分数
        ...
        return outputs
#对于transformers的基本自我注意力机制的扩展，引入结构化注意偏差，并采用两种变换计算该偏差
